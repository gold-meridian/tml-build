    ...
    private static BuildPurpose GetBuildPurpose(VersionCache cache, string tmlVersion, TaskLoggingHelper log)
    {
        ...
        if (ModLoaderVersion.TryParse(tmlVersion, out var version))
        {
            ...
            */
            ...
            var path = cache.GetVersionExtractDir(version);
            if (!Directory.Exists(path))
            {
                log.LogError($"Could not find tML version '{version}' in cache, defaulting to stable");
            }

            if (LocalBuildRevision.TryGetFromModule(new Logger(log), Path.Combine(path, "tModLoader.dll"), out var revision))
            {
                log.LogMessage("tML version determined from numeric version, determined purpose: " + revision.BuildPurpose);
                return revision.BuildPurpose;
            }

            log.LogError($"Could not determine tML version from numeric version '{version}', defaulting to stable");
            return BuildPurpose.Stable;
        }
        ...
        if (tmlVersion == "steam")
        {
            ...
            else if (LocalBuildRevision.TryGetFromModule(new Logger(log), Path.Combine(cache.SteamPath, "tModLoader.dll"), out var revision))
            {
