...
public static class VersionManager
{
    ...
    /// </summary>
    public static void WriteAliasProps(ILogWrapper log, VersionCache cache, string? aliasPropsFile = null)
    {
        ...
        // null paths aren't warnings but failure to parse is.
        var steamVersion = default(string);
        var steamPurpose = default(BuildPurpose?);
        if (cache.SteamPath is null)
        {
            log.Info("Steam path is null, skipping Steam version and purpose");
        }
        else if (LocalBuildRevision.TryGetFromModule(log, Path.Combine(cache.SteamPath, "tModLoader.dll"), out var revision))
        {
            steamVersion = revision.TmlVersion.ToString();
            steamPurpose = revision.BuildPurpose;
        }
        else
        {
            log.Warn("Failed to parse Steam tModLoader.dll, skipping Steam version and purpose");
        }

        var devVersion = default(string);
        var devPurpose = default(BuildPurpose?);
        if (cache.DevPath is null)
        {
            log.Info("Dev path is null, skipping dev version and purpose");
        }
        else if (LocalBuildRevision.TryGetFromModule(log, Path.Combine(cache.DevPath, "tModLoader.dll"), out var revision))
        {
            devVersion = revision.TmlVersion.ToString();
            devPurpose = revision.BuildPurpose;
        }
        else
        {
            log.Warn("Failed to parse Dev tModLoader.dll, skipping dev version and purpose");
        }
        ...
        var content = $"""
                       ...
                       <Project>
                           <PropertyGroup>
                               ...
                               <TmlVersionSteam>{steamVersion ?? ""}</TmlVersionSteam>
                               <TmlPurposeSteam>{(steamPurpose.HasValue ? steamPurpose : "")}</TmlPurposeSteam>
                               <TmlDevTargetsPath>{devTargets}</TmlDevTargetsPath>
                               <TmlVersionDev>{devVersion}</TmlVersionDev>
                               <TmlPurposeDev>{(devPurpose.HasValue ? devPurpose : "")}</TmlPurposeDev>
                           </PropertyGroup>
                       </Project>
                       """;

        var path = aliasPropsFile ?? Path.Combine(cache.RootDirectory, alias_props_file);
