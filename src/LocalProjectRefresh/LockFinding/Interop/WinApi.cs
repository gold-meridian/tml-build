...
public static partial class WinApi
{
    ...
    )
    {
        ...
        return res ? dupHandle : new SafeFileHandle(nint.Zero, true);
    }

    [LibraryImport("Kernel32.dll", EntryPoint = "GetCurrentProcess", SetLastError = true)]
    private static partial nint GetCurrentProcessPrivate();
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    public static partial bool OpenProcessToken(SafeProcessHandle processHandle, TokenAccessRights desiredAccess, out SafeFileHandle tokenHandle);

    [LibraryImport("Kernel32.dll", SetLastError = true)]
    public static partial FileType GetFileType(SafeFileHandle hFile);
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    public static partial bool EnumProcessModules(SafeProcessHandle hProcess, [Out] nint[] lphModule, int cb, out int lpcbNeeded);
