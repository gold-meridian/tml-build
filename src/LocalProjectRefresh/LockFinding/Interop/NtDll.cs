...
namespace LocalProjectRefresh.LockFinding.Interop;
...
public static partial class NtDll
{
    ...
    [LibraryImport("ntdll.dll", EntryPoint = "NtQueryObject")]
    ...
    public static string? GetHandleName(SafeFileHandle handle)
    {
        ...
        try
        {
            var name = Marshal.PtrToStructure<UNICODE_STRING>(buffer);
    ...
    public static string? GetHandleType(SafeFileHandle handle)
    {
        ...
        try
        {
            var info = Marshal.PtrToStructure<OBJECT_TYPE_INFORMATION>(buffer);
    ...
    private static nint QueryObjectInformation(SafeFileHandle handle, OBJECT_INFORMATION_CLASS informationClass, out int length)
    {
        length = 0;
        NtQueryObject(handle.DangerousGetHandle(), informationClass, nint.Zero, 0, ref length);
    ...
    private static SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX[] GetHandleEntries(SYSTEM_HANDLE_INFORMATION_EX handleInfo, nint buffer)
    {
        ...
        long handleSize = Marshal.SizeOf<SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX>();
        var current = new nint(buffer.ToInt64() + Marshal.SizeOf<SYSTEM_HANDLE_INFORMATION_EX>() - handleSize);
        ...
        for (long i = 0; i < numberOfHandles; i++)
        {
            ...
            current = new nint(current.ToInt64() + handleSize);
        }
